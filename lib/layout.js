...
class Layout {
  ...
  constructor (directory = '', globalLocals = {}, transform = null) {
  ...
  include (filePath, locals = {}) {
    ...
    delete this._parent;
    delete this.body;
    const result    = this.render(filePath, this.getTransformLocals(newLocals), {});
    this._parent    = parent;
    this.body       = body;
    this._directory = directory;
    return result;
    ...
    return function (filePath, locals, cb) {
      process.nextTick(() => {
        try {
          ...
          const layout      = new Layout(viewspath, mainGlobals, transform);
          const result      = layout.render(filePath, layout.getTransformLocals({}), {});
          cb(null, result);
        } catch (error) {
          cb(error);
        }
